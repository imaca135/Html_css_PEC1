...
})({"exEan":[function(require,module,exports) {
var HMR_HOST = null;
var HMR_PORT = null;
var HMR_SECURE = false;
var HMR_ENV_HASH = "4a236f9275d0a351";
module.bundle.HMR_BUNDLE_ID = "97aca6e39285b61a";
"use strict";
...
},{}],"j5dnz":[function(require,module,exports) {
(function(window, $) {
    ...
    SliderPro.prototype = {
        ...
        _init: function() {
            ...
            if (window.navigator.userAgent.match(/(iPad|iPhone|iPod)/g)) this.$slider.addClass('ios');
            // Check if IE (older than 11) is used and add the version number as a class to the slider since
            // older IE versions might need CSS tweaks.
            var rmsie = /(msie) ([\w.]+)/, ieVersion = rmsie.exec(window.navigator.userAgent.toLowerCase());
            if (this.isIE) this.$slider.addClass('ie');
            ...
            if (typeof modules !== 'undefined') for(var i = 0; i < modules.length; i++){
                var defaults = modules[i].substring(0, 1).toLowerCase() + modules[i].substring(1) + 'Defaults';
                if (typeof this[defaults] !== 'undefined') $.extend(this.defaults, this[defaults]);
            }
            ...
            if (typeof modules !== 'undefined') {
                for(var j = 0; j < modules.length; j++)if (typeof this['init' + modules[j]] !== 'undefined') this['init' + modules[j]]();
            }
            // Keep a reference of the original settings and use it
            // to restore the settings when the breakpoints are used.
            this.originalSettings = $.extend({
            }, this.settings);
            ...
            if (this.settings.breakpoints !== null) {
                for(var sizes in this.settings.breakpoints)this.breakpoints.push({
            ...
            $(window).on('resize.' + this.uniqueId + '.' + NS, function() {
                ...
                if (that.allowResize === false || that.previousWindowWidth === newWindowWidth && that.previousWindowHeight === newWindowHeight) return;
            ...
            if ($.isFunction(this.settings.init)) this.settings.init.call(this, {
                type: 'init'
            });
        },
        ...
        update: function() {
            ...
            if (this.settings.rightToLeft === true) this.$slider.addClass('sp-rtl');
            else this.$slider.removeClass('sp-rtl');
            ...
            for(var i = this.slides.length - 1; i >= 0; i--)if (this.$slider.find('.sp-slide[data-index="' + i + '"]').length === 0) {
            ...
            this.$slider.find('.sp-slide').each(function(index) {
                var $slide = $(this);
                if (typeof $slide.attr('data-init') === 'undefined') that._createSlide(index, $slide);
                else that.slides[index].setIndex(index);
                that.slidesOrder.push(index);
            });
            // Calculate the position/index of the middle slide
            this.middleSlidePosition = parseInt((that.slidesOrder.length - 1) / 2, 10);
            // Arrange the slides in a loop
            if (this.settings.loop === true) this._updateSlidesOrder();
            // Fire the 'update' event
            this.trigger({
                type: 'update'
            });
            if ($.isFunction(this.settings.update)) this.settings.update.call(this, {
                type: 'update'
            });
        },
        ...
        _createSlide: function(index, element) {
            ...
            slide.on('imagesLoaded.' + NS, function(event) {
                ...
                if (that.settings.autoHeight === true && event.index === that.selectedSlideIndex) that._resizeHeightTo(slide.getSize().height);
            });
        },
        // Arrange the slide elements in a loop inside the 'slidesOrder' array
        _updateSlidesOrder: function() {
            var slicedItems, i, // Calculate the distance between the selected element and the middle position
            ...
            if (distance < 0) {
                ...
                for(i = slicedItems.length - 1; i >= 0; i--)this.slidesOrder.unshift(slicedItems[i]);
            } else if (distance > 0) {
                slicedItems = this.slidesOrder.splice(0, distance);
                for(i = 0; i <= slicedItems.length - 1; i++)this.slidesOrder.push(slicedItems[i]);
            }
        },
        // Set the left/top position of the slides based on their position in the 'slidesOrder' array
        _updateSlidesPosition: function() {
            var selectedSlidePixelPosition = parseInt(this.$slides.find('.sp-slide').eq(this.selectedSlideIndex).css(this.positionProperty), 10), slide, $slideElement, slideIndex, previousPosition = selectedSlidePixelPosition, directionMultiplier, slideSize;
        ...
        _resetSlidesPosition: function() {
            var previousPosition = 0, slide, $slideElement, slideIndex, selectedSlideSize, directionMultiplier, slideSize;
            if (this.settings.autoSlideSize === true) {
        ...
        resize: function() {
            ...
            if (this.settings.breakpoints !== null && this.breakpoints.length > 0) {
                if ($(window).width() > this.breakpoints[this.breakpoints.length - 1].size && this.currentBreakpoint !== -1) {
                    ...
                    for(var i = 0, n = this.breakpoints.length; i < n; i++)if ($(window).width() <= this.breakpoints[i].size) {
                        if (this.currentBreakpoint !== this.breakpoints[i].size) {
                            ...
                            var settings = $.extend({
                            }, this.originalSettings, this.breakpoints[i].properties);
            ...
            if (this.settings.forceSize === 'fullWindow') this.slideHeight = $(window).height();
            ...
            else this.$slidesMask.css(this.vendorPrefix + 'transition', '');
            ...
            if (this.settings.visibleSize !== 'auto') {
                if (this.settings.orientation === 'horizontal') {
                    ...
                    if (this.settings.forceSize === 'fullWindow') this.$slider.css({
            ...
            if ($.isFunction(this.settings.sliderResize)) this.settings.sliderResize.call(this, {
                type: 'sliderResize'
            });
        },
        // Resize each individual slide
        _resizeSlides: function() {
            var slideWidth = this.slideWidth, slideHeight = this.slideHeight;
            if (this.settings.autoSlideSize === true) {
        ...
        _resizeHeight: function() {
            var that = this, selectedSlide = this.getSlideAt(this.selectedSlideIndex);
        ...
        gotoSlide: function(index) {
            ...
            var selectedSlideOffset = this.settings.centerSelectedSlide === true && this.settings.visibleSize !== 'auto' ? Math.round((parseInt(this.$slidesMask.css(this.sizeProperty), 10) - this.getSlideAt(this.selectedSlideIndex).getSize()[this.sizeProperty]) / 2) : 0, newSlidesPosition = -parseInt(this.$slides.find('.sp-slide').eq(this.selectedSlideIndex).css(this.positionProperty), 10) + selectedSlideOffset;
            ...
            this._moveTo(newSlidesPosition, false, function() {
                ...
                if ($.isFunction(that.settings.gotoSlideComplete)) that.settings.gotoSlideComplete.call(that, {
                    type: 'gotoSlideComplete',
                    index: index,
                    previousIndex: that.previousSlideIndex
                });
            ...
            if ($.isFunction(this.settings.gotoSlide)) this.settings.gotoSlide.call(this, {
                type: 'gotoSlide',
                index: index,
                previousIndex: this.previousSlideIndex
            });
        ...
        _moveTo: function(position, instant, callback) {
            ...
            if ((this.supportedAnimation === 'css-3d' || this.supportedAnimation === 'css-2d') && this.isIE === false) {
                ...
                if (this.supportedAnimation === 'css-3d') css[this.vendorPrefix + 'transform'] = 'translate3d(' + left + 'px, ' + top + 'px, 0)';
                else css[this.vendorPrefix + 'transform'] = 'translate(' + left + 'px, ' + top + 'px)';
                ...
                if (typeof instant !== 'undefined' && instant === true) this.$slides.css(css);
                else {
                    this.$slides.addClass('sp-animated');
        ...
        _resizeHeightTo: function(height) {
            ...
            if (this.supportedAnimation === 'css-3d' || this.supportedAnimation === 'css-2d') {
                ...
                this.$slidesMask.on(this.transitionEvent, function(event) {
                    ...
                    if ($.isFunction(that.settings.resizeHeightComplete)) that.settings.resizeHeightComplete.call(that, {
                        type: 'resizeHeightComplete'
                    });
                });
                this.$slidesMask.css(css);
            } else this.$slidesMask.stop().animate(css, this.settings.heightAnimationDuration, function(event) {
                // Fire the 'resizeHeightComplete' event
                that.trigger({
                    type: 'resizeHeightComplete'
                });
        ...
        destroy: function() {
            ...
            var modules = $.SliderPro.modules;
            if (typeof modules !== 'undefined') {
                for(var i = 0; i < modules.length; i++)if (typeof this['destroy' + modules[i]] !== 'undefined') this['destroy' + modules[i]]();
            }
            // Destroy all slides
            $.each(this.slides, function(index, element) {
                element.destroy();
        ...
        _setProperties: function(properties, store) {
            ...
            for(var prop in properties){
                ...
                if (store !== false) this.originalSettings[prop] = properties[prop];
            }
            this.update();
        },
    ...
    SliderProSlide.prototype = {
        ...
        _init: function() {
            ...
            if (this.$mainImage !== null) {
                ...
                if (this.$mainImage.parent('a').length !== 0) this.$mainImage.parent('a').appendTo(this.$imageContainer);
                else this.$mainImage.appendTo(this.$imageContainer);
            }
        ...
        setSize: function(width, height) {
            ...
            if (this.hasMainImage === true) {
                ...
                if (typeof this.$mainImage.attr('data-src') === 'undefined') this.resizeMainImage();
            }
        },
        ...
        calculateSize: function() {
            ...
            this.$slide.children().each(function(index, element) {
                var child = $(element);
                if (child.is(':hidden') === true) return;
        ...
        resizeMainImage: function(isNewImage) {
            ...
            if (this.settings.autoSlideSize === true) {
                ...
                if (this.settings.centerImage === true) this.$mainImage.css({
                    'marginLeft': (this.$imageContainer.width() - this.$mainImage.width()) * 0.5
                });
            } else {
                if (this.settings.imageScaleMode === 'cover') {
                    if (this.$mainImage.width() / this.$mainImage.height() <= this.$slide.width() / this.$slide.height()) this.$mainImage.css({
                        width: '100%',
                        height: 'auto'
                    });
                    else this.$mainImage.css({
                        width: 'auto',
                        height: '100%'
                    });
                ...
                } else if (this.settings.imageScaleMode === 'exact') this.$mainImage.css({
                    width: '100%',
                    height: '100%'
                });
                if (this.settings.centerImage === true) this.$mainImage.css({
    ...
    $.fn.sliderPro = function(options) {
        ...
        return this.each(function() {
            ...
            if (typeof $(this).data('sliderPro') === 'undefined') {
                ...
                if (typeof currentInstance[options] === 'function') currentInstance[options].apply(currentInstance, args);
                else if (typeof currentInstance.settings[options] !== 'undefined') {
                    var obj = {
                    };
                    obj[options] = args[0];
                    currentInstance._setProperties(obj);
                } else if (typeof options === 'object') currentInstance._setProperties(options);
                else $.error(options + ' does not exist in sliderPro.');
            }
        });
    };
    ...
    var SliderProUtils = {
        ...
        getSupportedAnimation: function() {
            ...
            if (isCSSTransitions === true) {
                ...
                if (typeof div.style.WebkitPerspective !== 'undefined' || typeof div.style.perspective !== 'undefined') this.supportedAnimation = 'css-3d';
                ...
                if (this.supportedAnimation === 'css-3d' && typeof div.styleWebkitPerspective !== 'undefined') {
                    ...
                    if (!(div.offsetLeft === 9 && div.offsetHeight === 5)) this.supportedAnimation = null;
        ...
        getVendorPrefix: function() {
            ...
            var div = document.createElement('div'), prefixes = [
                'Webkit',
                'Moz',
                'ms',
                'O'
            ...
            for(var i = 0; i < prefixes.length; i++)if (prefixes[i] + 'Transform' in div.style) {
        ...
        getTransitionEvent: function() {
            ...
            for(var transition in transitions)if (transition in div.style) {
        ...
        checkImagesComplete: function(target, callback) {
            var that = this, // Check the initial status of the image(s)
            status = this.checkImagesStatus(target);
            // If there are loading images, wait for them to load.
            // If the images are loaded, call the callback function directly.
            if (status === 'loading') var checkImages = setInterval(function() {
        ...
        checkImagesStatus: function(target) {
            var status = 'complete';
            if (target.is('img') && target[0].complete === false) status = 'loading';
            else target.find('img').each(function(index) {
                var image = $(this)[0];
                if (image.complete === false) status = 'loading';
            });
            return status;
        },
        checkIE: function() {
            ...
            if (userAgent.indexOf('MSIE') !== -1 || userAgent.match(/Trident.*rv\:11\./)) this.isIE = true;
...
(function(window, $) {
    ...
    var Thumbnails = {
        ...
        _thumbnailsOnUpdate: function() {
            ...
            if (this.$thumbnailsContainer === null) this.$thumbnailsContainer = $('<div class="sp-thumbnails-container"></div>').insertAfter(this.$slidesContainer);
            ...
            this.$slides.find('.sp-thumbnail').each(function(index) {
                ...
                if (thumbnailIndex > lastThumbnailIndex) $thumbnail.appendTo(that.$thumbnails);
                else $thumbnail.insertBefore(that.$thumbnails.find('.sp-thumbnail').eq(thumbnailIndex));
            });
            // Loop through the Thumbnail objects and if a corresponding element is not found in the DOM,
            // it means that the thumbnail might have been removed. In this case, destroy that Thumbnail instance.
            for(var i = this.thumbnails.length - 1; i >= 0; i--)if (this.$thumbnails.find('.sp-thumbnail[data-index="' + i + '"]').length === 0) {
            ...
            this.$thumbnails.find('.sp-thumbnail').each(function(index) {
                var $thumbnail = $(this);
                if (typeof $thumbnail.attr('data-init') === 'undefined') that._createThumbnail($thumbnail, index);
                else that.thumbnails[index].setIndex(index);
            });
            ...
            if (this.settings.thumbnailPointer === true) this.$thumbnailsContainer.addClass('sp-has-pointer');
            else this.$thumbnailsContainer.removeClass('sp-has-pointer');
            ...
            if ($.isFunction(this.settings.thumbnailsUpdate)) this.settings.thumbnailsUpdate.call(this, {
                type: 'thumbnailsUpdate'
            });
        },
        ...
        _thumbnailsOnResize: function() {
            ...
            if (this.thumbnailsOrientation === 'horizontal') {
                ...
                if (this.$slidesMask.width() + this.$thumbnailsContainer.outerWidth(true) > this.$slider.parent().width()) {
                    ...
                    if (this.settings.forceSize === 'fullWidth' || this.settings.forceSize === 'fullWindow') this.$slider.css('max-width', $(window).width() - this.$thumbnailsContainer.outerWidth(true));
                    else this.$slider.css('max-width', this.$slider.parent().width() - this.$thumbnailsContainer.outerWidth(true));
                    this.$slidesMask.css('width', this.$slider.width());
            ...
            if (this.thumbnailsSize <= this.thumbnailsContainerSize || this.$thumbnails.find('.sp-selected-thumbnail').length === 0) newThumbnailsPosition = 0;
            ...
            // for the thumbnails.
            if (this.settings.thumbnailsPosition === 'top') this.$slider.css({
                'paddingTop': this.$thumbnailsContainer.outerHeight(true),
                'paddingLeft': '',
                'paddingRight': ''
            });
            else if (this.settings.thumbnailsPosition === 'bottom') this.$slider.css({
                'paddingTop': '',
                'paddingLeft': '',
                'paddingRight': ''
            });
            else if (this.settings.thumbnailsPosition === 'left') this.$slider.css({
                'paddingTop': '',
                'paddingLeft': this.$thumbnailsContainer.outerWidth(true),
                'paddingRight': ''
            });
            else if (this.settings.thumbnailsPosition === 'right') this.$slider.css({
                'paddingTop': '',
                'paddingLeft': '',
        ...
        _gotoThumbnail: function(index) {
            ...
            if (this.settings.rightToLeft === true && this.thumbnailsOrientation === 'horizontal') {
                if (this.selectedThumbnailIndex >= previousIndex) {
                    var rtlNextThumbnailIndex = this.selectedThumbnailIndex === this.thumbnails.length - 1 ? this.selectedThumbnailIndex : this.selectedThumbnailIndex + 1, rtlNextThumbnail = this.thumbnails[rtlNextThumbnailIndex];
                    ...
                    var rtlPreviousThumbnailIndex = this.selectedThumbnailIndex === 0 ? this.selectedThumbnailIndex : this.selectedThumbnailIndex - 1, rtlPreviousThumbnail = this.thumbnails[rtlPreviousThumbnailIndex], rtlThumbnailsRightPosition = -this.thumbnailsPosition + this.thumbnailsContainerSize;
            ...
            if ($.isFunction(this.settings.gotoThumbnail)) this.settings.gotoThumbnail.call(this, {
                type: 'gotoThumbnail'
            });
        },
        ...
        _moveThumbnailsTo: function(position, instant, callback) {
            ...
            if (this.supportedAnimation === 'css-3d' || this.supportedAnimation === 'css-2d') {
                ...
                if (typeof instant !== 'undefined' && instant === true) transition = '';
                    ...
                    transition = this.vendorPrefix + 'transform ' + 0.7 + 's';
                    this.$thumbnails.on(this.transitionEvent, function(event) {
                        ...
                        if ($.isFunction(that.settings.thumbnailsMoveComplete)) that.settings.thumbnailsMoveComplete.call(that, {
                            type: 'thumbnailsMoveComplete'
                        });
                    });
                }
                ...
                if (typeof instant !== 'undefined' && instant === true) this.$thumbnails.css(css);
                else this.$thumbnails.addClass('sp-animated').animate(css, 700, function() {
                    that.$thumbnails.removeClass('sp-animated');
                    if (typeof callback === 'function') callback();
                    // Fire the 'thumbnailsMoveComplete' event
                    that.trigger({
    ...
    Thumbnail.prototype = {
        _init: function() {
            ...
            if (this.$thumbnail.parent('a').length !== 0) this.$thumbnail.parent('a').appendTo(this.$thumbnailContainer);
            else this.$thumbnail.appendTo(this.$thumbnailContainer);
        ...
        setSize: function(width, height) {
            ...
            if (this.$thumbnail.is('img') && typeof this.$thumbnail.attr('data-src') === 'undefined') this.resizeImage();
        },
        ...
        resizeImage: function() {
            ...
            if (imageWidth / imageHeight <= this.width / this.height) this.$thumbnail.css({
                width: '100%',
                height: 'auto'
            });
            else this.$thumbnail.css({
                width: 'auto',
                height: '100%'
            });
            this.$thumbnail.css({
        ...
        destroy: function() {
            ...
            if (this.$thumbnail.parent('a').length !== 0) this.$thumbnail.parent('a').insertBefore(this.$thumbnailContainer);
            else this.$thumbnail.insertBefore(this.$thumbnailContainer);
...
(function(window, $) {
    ...
    var ConditionalImages = {
        ...
        _conditionalImagesOnUpdate: function() {
            $.each(this.slides, function(index, element) {
                ...
                $slide.find('img:not([ data-default ])').each(function() {
                    var $image = $(this);
                    if (typeof $image.attr('data-src') !== 'undefined') $image.attr('data-default', $image.attr('data-src'));
                    else $image.attr('data-default', $image.attr('src'));
                });
            });
        },
        ...
        _conditionalImagesOnResize: function() {
            if (this.slideWidth <= this.settings.smallSize) this.currentImageSize = 'small';
            else if (this.slideWidth <= this.settings.mediumSize) this.currentImageSize = 'medium';
            else if (this.slideWidth <= this.settings.largeSize) this.currentImageSize = 'large';
            ...
            if (this.previousImageSize !== this.currentImageSize) {
                ...
                $.each(this.slides, function(index, element) {
                    ...
                    $slide.find('img').each(function() {
                        ...
                        {
                            if (typeof $image.attr('data-src') === 'undefined' && $image.attr('src') !== imageSource) that._loadConditionalImage($image, imageSource, function(newImage) {
        ...
        _loadConditionalImage: function(image, source, callback) {
            ...
            if (typeof image.attr('width') !== 'undefined') newImage.attr('width', image.attr('width'));
...
(function(window, $) {
    ...
    var Retina = {
        initRetina: function() {
            ...
            if (this._isRetina() === false) return;
            this.on('sliderResize.' + NS, $.proxy(this._checkRetinaImages, this));
            if (this.$slider.find('.sp-thumbnail').length !== 0) this.on('update.Thumbnails.' + NS, $.proxy(this._checkRetinaThumbnailImages, this));
        },
            ...
            $.each(this.slides, function(index, element) {
                ...
                if (typeof $slide.attr('data-retina-loaded') === 'undefined') {
                    ...
                    $slide.find('img[data-retina]').each(function() {
                        var $image = $(this);
                        if (typeof $image.attr('data-src') !== 'undefined') $image.attr('data-src', $image.attr('data-retina'));
                        else that._loadRetinaImage($image, function(newImage) {
                            if (newImage.hasClass('sp-image')) {
                                element.$mainImage = newImage;
                                element.resizeMainImage(true);
                            }
                ...
                if (typeof $thumbnail.attr('data-retina-loaded') === 'undefined') {
                    ...
                    $thumbnail.find('img[data-retina]').each(function() {
                        ...
                        if (typeof $image.attr('data-src') !== 'undefined') $image.attr('data-src', $image.attr('data-retina'));
                            ...
                            if (newImage.hasClass('sp-thumbnail')) element.resizeImage();
                        });
                    });
                }
            });
...
(function(window, $) {
    ...
    var LazyLoading = {
        ...
        _lazyLoadingOnResize: function() {
            ...
            if (this.$slider.find('.sp-thumbnail').length !== 0) this._checkAndLoadVisibleThumbnailImages();
        ...
        _checkAndLoadVisibleImages: function() {
            ...
            var that = this, // Use either the middle position or the index of the selected slide as a reference, depending on
        ...
        _checkAndLoadVisibleThumbnailImages: function() {
            ...
            var that = this, thumbnailSize = this.thumbnailsSize / this.thumbnails.length, // Calculate the indexes of the first and last thumbnail that will be checked
...
(function(window, $) {
    ...
    var Layers = {
        ...
        _layersOnUpdate: function(event) {
            ...
            $.each(this.slides, function(index, element) {
                ...
                this.$slide.find('.sp-layer:not([ data-layer-init ])').each(function() {
                    ...
                    if (typeof element.layers === 'undefined') element.layers = [];
        ...
        _layersOnResize: function() {
            ...
            if (useAutoScale === true && this.slideWidth < autoScaleReference) scaleRatio = that.slideWidth / autoScaleReference;
            else scaleRatio = 1;
            $.each(this.slides, function(index, slide) {
                if (typeof slide.layers !== 'undefined') $.each(slide.layers, function(index, layer) {
        ...
        _layersGotoSlide: function(index) {
            ...
            if (this.$slider.hasClass('sp-swiping') || typeof animatedLayers === 'undefined' || animatedLayers.length === 0) this.layersGotoSlideReference(index);
            else {
            ...
            $.each(animatedLayers, function(index, element) {
                ...
                if (element.isVisible() === true) {
                    ...
                    if (layerCounter === animatedLayers.length) {
                        ...
                        if ($.isFunction(that.settings.showLayersComplete)) that.settings.showLayersComplete.call(that, {
                            type: 'showLayersComplete',
                            index: index
                        });
                    }
                } else element.show(function() {
                    layerCounter++;
                    if (layerCounter === animatedLayers.length) {
                        that.trigger({
                            type: 'showLayersComplete',
            ...
            $.each(animatedLayers, function(index, element) {
                ...
                if (element.isVisible() === false) {
                    ...
                    if (layerCounter === animatedLayers.length) {
                        ...
                        if ($.isFunction(that.settings.hideLayersComplete)) that.settings.hideLayersComplete.call(that, {
                            type: 'hideLayersComplete',
                            index: index
                        });
                    }
                } else element.hide(function() {
                    layerCounter++;
                    if (layerCounter === animatedLayers.length) {
                        that.trigger({
                            type: 'hideLayersComplete',
    ...
    Layer.prototype = {
        ...
        _setStyle: function() {
            ...
            if (typeof this.data.width !== 'undefined') this.$layer.css('width', this.data.width);
        ...
        scale: function(ratio) {
            ...
            if (typeof this.horizontalPosition !== 'string') css[horizontalProperty] = this.horizontalPosition * this.scaleRatio;
            // If the position is not set to a percentage value, apply the scaling to the position
        ...
        show: function(callback) {
            ...
            if (this.supportedAnimation === 'javascript') this.$layer.stop().delay(delay).css({
                'opacity': 0,
                'visibility': 'visible'
            }).animate({
                'opacity': 1
                ...
                if (stayDuration !== -1) that.stayTimer = setTimeout(function() {
                    that.hide();
                    that.stayTimer = null;
                }, stayDuration);
        ...
        hide: function(callback) {
            ...
            // Animate the layers with CSS3 or with JavaScript
            if (this.supportedAnimation === 'javascript') this.$layer.stop().delay(delay).animate({
                'opacity': 0
            }, duration * 1000, function() {
                ...
                this.$layer.on(this.transitionEvent, function(event) {
                    ...
                    if (that.visible === false) that.$layer.css('visibility', 'hidden');
                    if (typeof callback !== 'undefined') callback();
                });
                this.delayTimer = setTimeout(function() {
                    that.$layer.css(target);
...
(function(window, $) {
    ...
    var Fade = {
        ...
        _fadeGotoSlide: function(index) {
            ...
            // use fade.
            if (this.$slider.hasClass('sp-swiping')) this.fadeGotoSlideReference(index);
            else {
                ...
                if (that.settings.loop === true) that._updateSlidesOrder();
                ...
                this._fadeSlideTo($nextSlide, 1, function() {
                    ...
                    $.each(that.slides, function(index, element) {
                        if (typeof element.$slide.attr('data-transitioning') !== 'undefined') allTransitionsComplete = false;
                    });
                ...
                if (this.settings.fadeOutPreviousSlide === true) this._fadeSlideTo($previousSlide, 0);
                if (this.settings.autoHeight === true) this._resizeHeight();
                // Fire the 'gotoSlide' event
                this.trigger({
                    type: 'gotoSlide',
        ...
        _fadeSlideTo: function(target, opacity, callback) {
            ...
            if (opacity === 1) target.attr('data-transitioning', true);
            // Use CSS transitions if they are supported. If not, use JavaScript animation.
            if (this.supportedAnimation === 'css-3d' || this.supportedAnimation === 'css-2d') {
        ...
        destroyFade: function() {
            this.off('update.' + NS);
            if (this.fadeGotoSlideReference !== null) this.gotoSlide = this.fadeGotoSlideReference;
        },
        fadeDefaults: {
...
(function(window, $) {
    ...
    var TouchSwipe = {
        ...
        initTouchSwipe: function() {
            ...
            if (this.settings.touchSwipe === false) return;
            ...
            this.$slidesMask.find('a').on('click.' + NS, function(event) {
                if (that.$slider.hasClass('sp-swiping')) event.preventDefault();
            });
        ...
        _onTouchMove: function(event) {
            ...
            {
                if (this.slidesPosition > this.touchStartPosition && this.selectedSlideIndex === 0 || this.slidesPosition < this.touchStartPosition && this.selectedSlideIndex === this.getTotalSlides() - 1) distance = distance * 0.2;
            }
        ...
        _onTouchEnd: function(event) {
            ...
            // of the swipe event.
            if (this.$slider.hasClass('sp-swiping')) setTimeout(function() {
            ...
            var selectedSlideOffset = this.settings.centerSelectedSlide === true && this.settings.visibleSize !== 'auto' ? Math.round((parseInt(this.$slidesMask.css(this.sizeProperty), 10) - this.getSlideAt(this.selectedSlideIndex).getSize()[this.sizeProperty]) / 2) : 0, oldSlidesPosition = -parseInt(this.$slides.find('.sp-slide').eq(this.selectedSlideIndex).css(this.positionProperty), 10) + selectedSlideOffset;
            if (Math.abs(touchDistance) < this.settings.touchSwipeThreshold) this._moveTo(oldSlidesPosition);
            else {
                ...
                if (this.settings.loop === true) this.gotoSlide(nextSlideIndex);
                else if (typeof nextSlideIndex !== 'undefined') this.gotoSlide(nextSlideIndex);
                else this._moveTo(oldSlidesPosition);
            }
        },
        // Destroy the module
...
(function(window, $) {
    ...
    var Caption = {
            ...
            if (this.settings.fadeCaption === true) {
                ...
                if (this.captionContent !== '') {
                    ...
                    this._fadeCaptionTo(0, function() {
                        ...
                        } else that.$captionContainer.empty();
                    });
                } else {
        ...
        _fadeCaptionTo: function(opacity, callback) {
            ...
            } else this.$captionContainer.stop().animate({
                'opacity': opacity
    ...
    var DeepLinking = {
        initDeepLinking: function() {
            ...
            this.on('gotoSlide.' + NS, function(event) {
                if (that.settings.updateHash === true) {
                    ...
                    if (typeof slideId === 'undefined') slideId = event.index;
        ...
        _parseHash: function(hash) {
            if (hash !== '') {
                ...
                if (this.$slider.attr('id') === sliderId) return {
                    'sliderID': sliderId,
                    'slideId': slideId
                };
            }
        ...
        _gotoHash: function(hash) {
            ...
            if (isNaN(slideIdNumber)) {
                ...
                if (slideIndex !== -1 && slideIndex !== this.selectedSlideIndex) this.gotoSlide(slideIndex);
            } else if (slideIdNumber !== this.selectedSlideIndex) this.gotoSlide(slideIdNumber);
        },
        // Destroy the module
    ...
    var Autoplay = {
        ...
        startAutoplay: function() {
            ...
            this.autoplayTimer = setTimeout(function() {
                if (that.settings.autoplayDirection === 'normal') that.nextSlide();
                else if (that.settings.autoplayDirection === 'backwards') that.previousSlide();
...
(function(window, $) {
    ...
    var Keyboard = {
        initKeyboard: function() {
            ...
            if (this.settings.keyboard === false) return;
            ...
            $(document).on('keydown.' + this.uniqueId + '.' + NS, function(event) {
                ...
                if (event.which === 37) that.previousSlide();
                else if (event.which === 39) that.nextSlide();
...
(function(window, $) {
    ...
    var FullScreen = {
        ...
        _fullScreenOnUpdate: function() {
            if (this.settings.fullScreen === true && this.$fullScreenButton === null) this._addFullScreen();
            else if (this.settings.fullScreen === false && this.$fullScreenButton !== null) this._removeFullScreen();
            if (this.settings.fullScreen === true) {
                if (this.settings.fadeFullScreen === true) this.$fullScreenButton.addClass('sp-fade-full-screen');
                else if (this.settings.fadeFullScreen === false) this.$fullScreenButton.removeClass('sp-fade-full-screen');
            }
        },
        ...
        _onFullScreenButtonClick: function() {
            if (this.isFullScreen === false) {
                if (this.instance.requestFullScreen) this.instance.requestFullScreen();
                else if (this.instance.mozRequestFullScreen) this.instance.mozRequestFullScreen();
                else if (this.instance.webkitRequestFullScreen) this.instance.webkitRequestFullScreen();
                else if (this.instance.msRequestFullscreen) this.instance.msRequestFullscreen();
            } else {
...
(function(window, $) {
    ...
    var Buttons = {
        ...
        _buttonsOnUpdate: function() {
            ...
            if (this.settings.buttons === true && this.getTotalSlides() > 1 && this.$buttons.length === 0) this._createButtons();
            else if (this.settings.buttons === true && this.getTotalSlides() !== this.$buttons.find('.sp-button').length && this.$buttons.length !== 0) this._adjustButtons();
            else if (this.settings.buttons === false || this.getTotalSlides() <= 1 && this.$buttons.length !== 0) this._removeButtons();
        },
        // Create the buttons
        _createButtons: function() {
            ...
            // Create the buttons
            for(var i = 0; i < this.getTotalSlides(); i++)$('<div class="sp-button"></div>').appendTo(this.$buttons);
...
(function(window, $) {
    ...
    var Arrows = {
        ...
        _arrowsOnUpdate: function() {
            ...
            } else if (this.settings.arrows === false && this.$arrows !== null) this._removeArrows();
            if (this.settings.arrows === true) {
                if (this.settings.fadeArrows === true) this.$arrows.addClass('sp-fade-arrows');
                else if (this.settings.fadeArrows === false) this.$arrows.removeClass('sp-fade-arrows');
            }
        },
        // Show or hide the arrows depending on the position of the selected slide
        _checkArrowsVisibility: function() {
            if (this.settings.arrows === false || this.settings.loop === true) return;
            if (this.selectedSlideIndex === 0) this.$previousArrow.css('display', 'none');
            else this.$previousArrow.css('display', 'block');
            if (this.selectedSlideIndex === this.getTotalSlides() - 1) this.$nextArrow.css('display', 'none');
            else this.$nextArrow.css('display', 'block');
        },
...
(function(window, $) {
    ...
    var ThumbnailTouchSwipe = {
        ...
        _onThumbnailTouchEnd: function(event) {
            ...
            if (this.thumbnailsPosition > 0) this._moveThumbnailsTo(0);
            else if (this.thumbnailsPosition < this.thumbnailsContainerSize - this.thumbnailsSize) this._moveThumbnailsTo(this.thumbnailsContainerSize - this.thumbnailsSize);
            // Fire the 'thumbnailsMoveComplete' event
            this.trigger({
                type: 'thumbnailsMoveComplete'
            });
            if ($.isFunction(this.settings.thumbnailsMoveComplete)) this.settings.thumbnailsMoveComplete.call(this, {
                type: 'thumbnailsMoveComplete'
            });
        },
        // Destroy the module
    ...
    var ThumbnailArrows = {
        ...
        initThumbnailArrows: function() {
            ...
            this.on('sliderResize.' + NS + ' ' + 'thumbnailsMoveComplete.' + NS, function() {
                if (that.isThumbnailScroller === true && that.settings.thumbnailArrows === true) that._checkThumbnailArrowsVisibility();
            });
        },
        // Called when the slider is updated
        _thumbnailArrowsOnUpdate: function() {
            ...
            } else if (this.settings.thumbnailArrows === false && this.$thumbnailArrows !== null) this._removeThumbnailArrows();
            // Add fading functionality and check if the arrows need to be visible or not
            if (this.settings.thumbnailArrows === true) {
                if (this.settings.fadeThumbnailArrows === true) this.$thumbnailArrows.addClass('sp-fade-thumbnail-arrows');
                else if (this.settings.fadeThumbnailArrows === false) this.$thumbnailArrows.removeClass('sp-fade-thumbnail-arrows');
        ...
        _checkThumbnailArrowsVisibility: function() {
            if (this.thumbnailsPosition === 0) this.$previousThumbnailArrow.css('display', 'none');
            else this.$previousThumbnailArrow.css('display', 'block');
            if (this.thumbnailsPosition === this.thumbnailsContainerSize - this.thumbnailsSize) this.$nextThumbnailArrow.css('display', 'none');
            else this.$nextThumbnailArrow.css('display', 'block');
        },
    ...
    var Video = {
        ...
        _initVideo: function(video) {
            ...
            video.on('videoPlay.' + NS, function() {
                ...
                if ($.isFunction(that.settings.videoPlay)) that.settings.videoPlay.call(that, eventObject);
            });
            ...
            video.on('videoPause.' + NS, function() {
                ...
                that.trigger(eventObject);
                if ($.isFunction(that.settings.videoPause)) that.settings.videoPause.call(that, eventObject);
            });
            ...
            video.on('videoEnded.' + NS, function() {
                if (that.settings.endVideoAction === 'startAutoplay' && typeof that.startAutoplay !== 'undefined') {
                    ...
                    that.startAutoplay();
                    that.settings.autoplay = true;
                } else if (that.settings.endVideoAction === 'nextSlide') that.nextSlide();
                else if (that.settings.endVideoAction === 'replayVideo') video.videoController('replay');
                ...
                that.trigger(eventObject);
                if ($.isFunction(that.settings.videoEnd)) that.settings.videoEnd.call(that, eventObject);
            });
        },
        ...
        _preinitVideo: function(video) {
            ...
            video.on('click.' + NS, function(event) {
                ...
                event.preventDefault();
                var href = video.attr('href'), iframe, provider, regExp, match, id, src, videoAttributes, videoWidth = video.children('img').attr('width') || video.children('img').width(), videoHeight = video.children('img').attr('height') || video.children('img').height();
        ...
        _videoOnGotoSlide: function(event) {
            ...
            if (event.previousIndex !== -1 && previousVideo.length !== 0) {
                if (this.settings.leaveVideoAction === 'stopVideo') previousVideo.videoController('stop');
                else if (this.settings.leaveVideoAction === 'pauseVideo') previousVideo.videoController('pause');
                    ...
                    } else previousVideo.videoController('stop');
                }
            }
        },
        // Called when a new slide is selected, 
        ...
        _videoOnGotoSlideComplete: function(event) {
            ...
            if (this.settings.reachVideoAction === 'playVideo' && event.index === this.selectedSlideIndex) {
                ...
                if (loadedVideo.length !== 0) loadedVideo.videoController('play');
                else if (unloadedVideo.length !== 0) unloadedVideo.trigger('click.' + NS);
...
(function($) {
    ...
    VideoController.prototype = {
        _init: function() {
            ...
            for(var name in players)if (typeof players[name] !== 'undefined' && players[name].isType(this.$video)) {
            ...
            $.each(events, function(index, element) {
                ...
                that.player.on(element, function() {
                    ...
                    if ($.isFunction(that.settings[event])) that.settings[event].call(that, {
                        type: event,
                        video: videoID
                    });
                });
    ...
    $.fn.videoController = function(options) {
        ...
        return this.each(function() {
            ...
            if (typeof $(this).data('videoController') === 'undefined') {
                ...
                // Check the type of argument passed
                if (typeof currentInstance[options] === 'function') currentInstance[options].apply(currentInstance, args);
                else $.error(options + ' does not exist in videoController.');
            }
        });
    };
    ...
    var YoutubeVideo = function(video) {
        ...
        if (typeof youtubeAPILoaded !== 'undefined') Video.call(this, video);
        else {
    ...
    YoutubeVideo.prototype.on = function(type, callback) {
        var that = this;
        if (this.init === true) Video.prototype.on.call(this, type, callback);
        else var timer = setInterval(function() {
            if (that.init === true) {
                clearInterval(timer);
    ...
    var VimeoVideo = function(video) {
        this.init = false;
        if (typeof window.Vimeo !== 'undefined') Video.call(this, video);
        else {
    ...
    JWPlayerVideo.prototype._init = function() {
        ...
        if (this.$video.hasClass('jwplayer')) videoID = this.$video.attr('id');
        else if (typeof this.$video.attr('data-jwplayer-id') !== 'undefined') videoID = this.$video.attr('data-jwplayer-id');
        else if (this.$video.find("object[data*='jwplayer']").length !== 0) videoID = this.$video.find('object').attr('id');
        // Get a reference to the player
...
},{}]},["exEan"], null, "parcelRequiree815")

//# sourceMappingURL=jquery.sliderPro.9285b61a.js.map
